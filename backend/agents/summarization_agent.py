"""
Summarization Agent - Summarizes each paper using Groq LLM (WRITER API key)
"""
import asyncio
import logging
from typing import Dict, List
from utils.groq_client import groq_json_writer

logger = logging.getLogger(__name__)


async def summarize_paper(paper: Dict) -> Dict:
    """Use Groq (writer key) to extract structured information from a paper."""
    title     = paper.get("title", "Unknown")
    abstract  = paper.get("abstract", "") or ""
    full_text = paper.get("full_text", "") or ""

    content = f"Title: {title}\n\nAbstract: {abstract}"
    if full_text and full_text.strip() != abstract.strip():
        content += f"\n\nExtracted Content:\n{full_text[:2500]}"

    try:
        summary = await groq_json_writer(
            system_prompt=(
                "You are an expert academic researcher who reads papers and extracts "
                "structured information for literature reviews. "
...
async def run_summarization_agent(papers: List[Dict], job_id: str, state_manager) -> List[Dict]:
    """Summarize all papers using the WRITER API key."""
    state_manager.update(job_id, current_agent="🧠 Summarization Agent", progress=35)
    state_manager.add_log(job_id, f"Summarizing {len(papers)} papers with Groq LLM (writer key)")
    ...
    async def process_one(paper, idx):
        async with semaphore:
            ...
            state_manager.add_log(
                job_id,
                f"[{idx+1}/{len(papers)}] Summarized: {paper.get('title', '')[:60]}"
            )
            state_manager.update(job_id, progress=35 + int(20 * (idx + 1) / len(papers)))
            await asyncio.sleep(0.5)
            return result

    summarized = await asyncio.gather(*[process_one(p, i) for i, p in enumerate(papers)])
